...
FILES
    Work Directory Structure:
        work-dir/
        âââ Dockerfile
        âââ build-output/Sparrow/        # Built artifacts
        âââ official/Sparrow/             # Official release
        âââ extracted-modules-comparison/ # JIMAGE extraction
        âââ modules-diff.txt              # Modules comparison results
        âââ file-list-diff.txt            # File listing comparison
        âââ docker-build.log              # Build log

REPRODUCIBILITY CRITERIA
    REPRODUCIBLE if:
        â Critical binaries byte-for-byte identical
        â Application code 100% identical
        â Only JVM infrastructure or legal files differ

    NOT REPRODUCIBLE if:
        â Critical binaries differ
        â Application code differs
        â Unexpected differences in dependencies

KNOWN ACCEPTABLE DIFFERENCES
    â¢ Missing legal files (~48 files in lib/runtime/legal/)
      Reason: JDK packaging differences (documentation only)

    â¢ Modules file size difference (~94 bytes, 0.0001%)
...
VERSION
    verify_sparrow.sh version v0.7.0
...
verify_critical_binaries() {
    ...
    for file in "${critical_files[@]}"; do
        ...
        # Show and execute sha256sum commands
        echo "$ sha256sum $build_dir/$file"
        sha256sum "$build_dir/$file"
        echo ""

        echo "$ sha256sum $official_dir/$file"
        sha256sum "$official_dir/$file"
        echo ""
...
analyze_file_counts() {
    ...
    echo "$ tree -L 3 $build_dir"
    if command -v tree &> /dev/null; then
        tree -L 3 "$build_dir"
    else
        find "$build_dir" -maxdepth 3 -print | sed -e "s;$build_dir;.;g;s;[^/]*\/;|__;g;s;__|; |;g"
    fi
    echo ""

    echo "$ tree -L 3 $official_dir"
    if command -v tree &> /dev/null; then
        tree -L 3 "$official_dir"
    else
        find "$official_dir" -maxdepth 3 -print | sed -e "s;$official_dir;.;g;s;[^/]*\/;|__;g;s;__|; |;g"
    fi
    echo ""

    # Count files
    echo "$ find $build_dir -type f | wc -l"
    find "$build_dir" -type f | wc -l
    echo ""

    echo "$ find $official_dir -type f | wc -l"
    find "$official_dir" -type f | wc -l
    echo ""

    echo "$ diff <(cd $build_dir && find . -type f | sort) <(cd $official_dir && find . -type f | sort) > file-list-diff.txt"
...
analyze_legal_files() {
    ...
    # Count legal files
    echo "$ find $build_dir/lib/runtime/legal/ -type f | wc -l"
    local build_legal
    build_legal=$(find "$build_dir/lib/runtime/legal/" -type f 2>/dev/null | wc -l | tr -d ' \n')
    build_legal=${build_legal:-0}
    echo "$build_legal"
    echo ""

    echo "$ find $official_dir/lib/runtime/legal/ -type f | wc -l"
    local official_legal
    official_legal=$(find "$official_dir/lib/runtime/legal/" -type f 2>/dev/null | wc -l | tr -d ' \n')
    official_legal=${official_legal:-0}
    echo "$official_legal"
    echo ""
    ...
    if [[ $missing_legal -gt 0 ]]; then
        ...
        non_legal_missing=${non_legal_missing:-0}
...
inspect_modules_file() {
    ...
    # Compare file sizes
    echo "$ ls -l $build_modules"
    ls -l "$build_modules"
    echo ""

    echo "$ ls -l $official_modules"
    ls -l "$official_modules"
    echo ""

    # Compare hashes
    echo "$ sha256sum $build_modules"
    sha256sum "$build_modules"
    echo ""

    echo "$ sha256sum $official_modules"
    sha256sum "$official_modules"
    echo ""
    ...
    echo "$ jimage extract --dir $extract_dir/build $build_modules"
    if ! jimage extract --dir "$extract_dir/build" "$build_modules"; then
    ...
    echo "$ jimage extract --dir $extract_dir/official $official_modules"
    if ! jimage extract --dir "$extract_dir/official" "$official_modules"; then
    ...
    echo "$ diff -qr $extract_dir/build $extract_dir/official > modules-diff.txt"
    diff -qr "$extract_dir/build" "$extract_dir/official" > "$WORK_DIR/modules-diff.txt" 2>&1 || true
    echo ""

    # Count total differences
    local total_diffs
    total_diffs=$(grep -c "^Files" "$WORK_DIR/modules-diff.txt" 2>/dev/null | tr -d ' \n')
    total_diffs=${total_diffs:-0}
    ...
    echo "$ diff -qr $extract_dir/build/com.sparrowwallet.sparrow $extract_dir/official/com.sparrowwallet.sparrow"
    if diff -qr "$extract_dir/build/com.sparrowwallet.sparrow" \
               "$extract_dir/official/com.sparrowwallet.sparrow" 2>&1; then
        echo "[NO OUTPUT - Files are IDENTICAL]"
        echo ""
        MODULES_INSPECTED=true
        APP_CODE_IDENTICAL=true
    else
    ...
    local jvm_diffs
    jvm_diffs=$(grep "java.base/java/lang/invoke" "$WORK_DIR/modules-diff.txt" 2>/dev/null | wc -l | tr -d ' \n')
    jvm_diffs=${jvm_diffs:-0}
...
generate_verdict() {
    ...
    if [[ "$CRITICAL_BINARIES_MATCH" == true ]]; then
        echo "  â Critical binaries: IDENTICAL"
    else
        echo "  â Critical binaries: DIFFER"
    fi

    if [[ "$APP_CODE_IDENTICAL" == true ]]; then
        echo "  â Application code: IDENTICAL"
    else
        echo "  â Application code: DIFFER"
    fi

    echo ""
    echo "Acceptable Differences:"
    if [[ "$JVM_CLASSES_ACCEPTABLE" == true ]]; then
        echo "  â JVM infrastructure: Expected differences only"
    else
        echo "  â  JVM infrastructure: Unexpected differences"
    fi

    if [[ "$LEGAL_FILES_ACCEPTABLE" == true ]]; then
        echo "  â Legal files: Acceptable (documentation only)"
    else
        echo "  â  Legal files: Requires review"
    fi
    ...
    if [[ "$CRITICAL_BINARIES_MATCH" == true ]] && [[ "$APP_CODE_IDENTICAL" == true ]]; then
        ...
        echo "VERDICT: â NOT REPRODUCIBLE (Critical binaries differ)"
        echo "======================================================"
        VERDICT_EXIT_CODE=$EXIT_CRITICAL_BINARIES_DIFFER
    elif [[ "$APP_CODE_IDENTICAL" == false ]]; then
        echo "VERDICT: â NOT REPRODUCIBLE (Application code differs)"
        echo "======================================================"
        VERDICT_EXIT_CODE=$EXIT_APP_CODE_DIFFERS
    else
        echo "VERDICT: â ï¸ MANUAL REVIEW REQUIRED"
        echo "======================================================"
        echo ""
