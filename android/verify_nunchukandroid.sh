...
# ==============================================================================
# Version:       v0.5.6 (Memory Fix)
# Organization:  WalletScrutiny.com
# Last Modified: 2025-11-13
# Project:       https://github.com/nunchuk-io/nunchuk-android
# ==============================================================================
# MEMORY FIX: Increased JVM heap from 6GB to 8GB and MetaSpace from 512MB to 1GB
# This prevents Gradle daemon crashes during Kotlin compilation
# ==============================================================================
...
check_memory() {
  ...
  if [[ $available_mem_gb -lt 6 ]]; then
    ...
    echo -e "${YELLOW}  Available: ${available_mem_gb}GB (Recommended: 8GB+)${NC}"
    ...
    if [[ $total_mem_gb -lt 12 ]]; then
      echo -e "${RED}${WARNING_ICON} Total RAM: ${total_mem_gb}GB (Minimum: 12GB recommended)${NC}"
      echo ""
    fi
...
if [ "$showScriptVersion" = true ]; then
  echo "verify_nunchukandroid.sh v0.5.6 (Memory Fix)"
  exit 0
fi
...
# Configure Gradle JVM args with INCREASED MEMORY to prevent daemon crashes
echo "[Container] Configuring Gradle JVM settings (8GB heap)..."
echo "org.gradle.jvmargs=-Xmx8g -XX:MaxMetaspaceSize=1024m -XX:+HeapDumpOnOutOfMemoryError -XX:+UseG1GC" >> gradle.properties
...
./gradlew clean bundleProductionRelease --no-daemon --max-workers=2
...
build_and_verify() {
  ...
  # Build container image with increased memory
  echo "Building container image from Nunchuk's Dockerfile..."
  $CONTAINER_CMD build --memory=8g --no-cache --ulimit nofile=65536:65536 -t nunchuk-verifier:${appVersion} -f Dockerfile .
  ...
  # Run container with all verification steps and increased memory
