#!/bin/bash
# verify_keystone3_pro_build.sh v1.0.0 - Standalone verification script for Keystone3 Pro Hardware Wallet
# Usage: verify_keystone3_pro_build.sh --version version [--type type] [-c]
...
usage() {
  echo 'NAME
       verify_keystone3_pro_build.sh - verify Keystone3 Pro hardware wallet firmware

SYNOPSIS
       verify_keystone3_pro_build.sh --version version [--type type] [-c]

DESCRIPTION
       This command tries to verify firmware builds of Keystone3 Pro hardware wallet.

       --version The firmware version to verify (e.g., 2.2.14)
       --type Firmware type: multicoin|cypherpunk|btc (default: multicoin)
       -c|--cleanup Clean up temporary files after testing

EXAMPLES
       verify_keystone3_pro_build.sh --version 2.2.14
       verify_keystone3_pro_build.sh --version 2.2.14 --type cypherpunk
       verify_keystone3_pro_build.sh --version 2.2.14 --type btc -c'
}
...
compare_firmware() {
  ...
  # Extract the unsigned firmware checksum from firmware checker output
  unsignedHash=$(./tools/code/firmware-checker/target/debug/fmc --source keystone3-unsigned.bin 2>/dev/null | grep "Firmware checksum sha256:" | awk '{print $4}')
  
  echo "Unsigned firmware hash: $unsignedHash"
  
  if [[ "$builtHash" == "$unsignedHash" ]]; then
    echo -e "${GREEN}✓ REPRODUCIBLE: Built firmware matches unsigned content${NC}"
    verdict="reproducible"
    result_status="1"
    status_text="MATCHES"
  else
    echo -e "${RED}✗ NOT REPRODUCIBLE: Built firmware differs from unsigned content${NC}"
    verdict="not_reproducible"
    result_status="0"
    status_text="DOESN'T MATCH"
  fi
...
generate_results() {
  echo "Generating COMPARISON_RESULTS.txt..."
  
  # Generate the required COMPARISON_RESULTS.txt file
  {
    echo "# Keystone3 Pro Firmware Verification Results"
    echo "# Format: filename - type - hash - status"
    echo ""
    echo "keystone3-pro-v${version}-${firmwareType}.bin - ${firmwareType} - ${builtHash} - ${result_status} (${status_text})"
...
result() {
  ...
  if [[ "$verdict" == "reproducible" ]]; then
    ...
    echo ""
    echo "✗ The firmware build is not reproducible."
    echo "- Official firmware hash: $officialHash"
    echo "- Built firmware hash: $builtHash"
    echo "- Unsigned content differs from build output"
  fi
  ...
  if [ "$shouldCleanup" = false ]; then
    ...
    echo "- Results file: COMPARISON_RESULTS.txt"
  fi
}
