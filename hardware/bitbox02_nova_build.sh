#!/bin/bash
# bitbox02_nova_build.sh v2.0.0 - Standardized verification script for BitBox02 Nova Hardware Wallet
# Follows WalletScrutiny reproducible verification standards
# Usage: bitbox02_nova_build.sh --version VERSION [--type TYPE]
...
BUILD_TYPE="firmware"
workDir="$(pwd)/bitbox02-nova-work"
...
usage() {
  echo 'NAME
       bitbox02_nova_build.sh - verify BitBox02 Nova hardware wallet firmware

SYNOPSIS
       bitbox02_nova_build.sh --version VERSION [--type TYPE]
...
EXAMPLES
       bitbox02_nova_build.sh --version 9.23.3
       bitbox02_nova_build.sh --version 9.23.3 --type multi'
}
...
results:
  - architecture: arm-cortex-m4
    ...
    files:
      - filename: firmware-bitbox02nova-${firmwareType}.v${version}.signed.bin
        hash: ${hash}
        match: ${match}
